	...
	$("#council-list-2019").change(function()
	{
		manageChanges($(this));
	});

	// function to populate wards for a given council
	$("#council-list-2014").change(function()
	{
		manageChanges($(this));
	});
	...
	$('#yearSelect').change(function() {
		...
		{
			case '2014':
				$("#council-list-2019").hide();
				$("#council-list-2014").show();
				$("#council-list-2014 option").filter('[value="0"]').prop("selected", true);
				$("#constituencySelect").empty();
				break;
			case '2019':
				$("#council-list-2014").hide();
				$("#council-list-2019").show();
				$("#council-list-2019 option").filter('[value="0"]').prop("selected", true);
				$("#constituencySelect").empty();
				break;
		}
...
function loadWards(year, council, selected)
{
	...
	var path = '/' + year + "/EU/" + council + "/all-constituency-info.json"; 
	$.getJSON(path, function(data) 
	{
		var constituencies = data.Constituencies.sort(cmpNames);
		var constituencySelect = $("#constituencySelect");
		$(constituencySelect).append($("<option/>").val(0).text("Select a region"));
...
function updateTitle (ward, council)
{
	...
	document.title = ward + ", " + council + " results visualisation for the European Parliament elections (Ireland)";
}
