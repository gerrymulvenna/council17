...
function europarl_head($title, $name, $twimg)
{
    ...
    {
        $url = "http://" . $_SERVER['SERVER_NAME'] . "/europarl/?year=" . $_GET['year'] . "&region=" . $_GET['region'];
        if (isset($regions[$_GET['region']]))
        {
            $desc = $regions[$_GET['region']] . " results visualisation for the European Parliament election " . $_GET['year'];
            $title = $desc;
        }
    }
    echo '<!DOCTYPE html>
...
';
echo regionList("region-list-2019", "select", "2019", "../2019/IRL");
echo regionList("region-list-2014", "select", "2014", "../2014/IRL");
...
echo '
            ...
            <div id="results-ack">
                ...
                <em>The UK candidate data was collated by</em></div><div id="dc-logo"><a href="http://democracyclub.org.uk"><img src="/website/image/democracyclub-logo-with-text.png" width="250"></a>
            </div>
        </div>
    </div>
';
...
{
    echo"<header><h1><a href = \"/\">$title</a></h1><p>Explore results of the European Parliament election in Ireland</p></header>\n";
    echo'
...
function selectRegion ($prompt, $class = "select", $suffix = ".php")
{
echo'<div id="select-region">
				<select id="region-list" class="' . $class . '" onClick="selectRegion()" name="region">';
echo "\n<option>$prompt</option>\n";
echo '					<option value="dublin' . $suffix . '">Dubin</option>
					<option value="midlands-northwest' . $suffix . '">Midlands/North-west</option>
					<option value="north' . $suffix . '">North</option>
					<option value="south' . $suffix . '">South</option>
				</select>
			</div>';
...
function regionList ($id, $class, $name, $root)
{
    $regions = array(
    "dublin" => "Dublin",
    "midlands-northwest"=> "Midlands/North-west",
    ...
    foreach ($dirlist as $path)
    {
        ...
        {
            if (is_dir($root . DIRECTORY_SEPARATOR . $path) && in_array($path, array_keys($regions)))
            {
                $fname = $root . DIRECTORY_SEPARATOR . $path . DIRECTORY_SEPARATOR . "all-constituency-info.json";
                if (file_exists($fname))
                {
                    $html .= '<option value="' . $path . '">' . $regions[$path] . '</option>' . "\n";
                }
            }
        }
    }
