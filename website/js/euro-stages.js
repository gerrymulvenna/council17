...
function animateStages(year,region) {
	setSearchParams (year,region);
	console.log ("animateStages", year,region);
    ...
    var json = (function() {
            ...
            $.ajax({
                'async': false,
                'global': false,
                'url': '/' + year + '/IRL/' + region + '/ResultsJson.json?' + new Date().getTime(),
                'dataType': "json",
    ...
    if(constituency){
        ...
        $("#quota").html("<p>Electorate: " + numberWithCommas(parseInt(constituency["Total_Electorate"])) + ", Turnout: " + numberWithCommas(parseInt(constituency["Total_Poll"])) + " (" + turnout + "%), Valid votes: " + numberWithCommas(parseInt(constituency["Valid_Poll"])) + ", Quota: " + numberWithCommas(quota) + "</p>\n");
    ...
    function firstCount(){
        ...
        for(var j=0;j<candidates.length;j++){
             $('<div id="cname'+candidates[j].id+'" class="candidateLabel '+candidates[j]["party"]+'_label" style="top:'+(topMargin+ (j*30)) +'px;left:10px;"><span title="'+candidates[j].title + '">'+candidates[j]["name"]+'</span></div>')
            .appendTo("#animation");
