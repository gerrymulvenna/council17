...
function selectRegion() {
	...
	if (year && region)
	{
		loadCandidates(region, year);
	}
};


$(yearSelect).on('change', function() {
	loadCandidates($("#region-list :selected").val(), this.value);
	$('#pastebin').val('');
});

function loadCandidates(region, year)
{
	...
	var names;
	var countdata = getCountInfo(region, year);
	...
	var fname = 'europarl.' + region + '.' + year + '.json';
...
function getCountInfo(region, year) {
	var countdata = [];
    var request = new XMLHttpRequest();
    var path = '/' + year + '/IRL/' + region + '/' + '/ResultsJson.json?' + new Date().getTime(); // add ? with timestamp to force XMLHttpRequest not to cache
	console.log(path);
    request.onreadystatechange = function() {
        if (request.readyState == 4 && request.status >= 200 && request.status < 400) {
